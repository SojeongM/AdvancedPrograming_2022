...
def waypoint_prediction(roadside1_spline, roadside2_spline, num_waypoints=6, way_type = "smooth"):
    ...
    if way_type == "center":
     ...
     #   way_points = np.zeros((2, num_waypoints))
        ...
        boundary_lane_1 = np.array(splev(t, roadside1_spline))
        boundary_lane_2 = np.array(splev(t, roadside2_spline))

        

        way_points=np.hstack(((boundary_lane_1[0]+boundary_lane_2[0])/2, (boundary_lane_1[1]+boundary_lane_2[1])/2))
        ...
        boundary_lane_1 = np.array(splev(t, roadside1_spline))
        boundary_lane_2 = np.array(splev(t, roadside2_spline))
        t1=boundary_lane_1[0]+boundary_lane_2[0]
        t2=boundary_lane_1[1]+boundary_lane_2[1]
        way_points_smooth=np.hstack((t1/2, t2/2))

        way_points = minimize(smoothing_objective, (way_points_smooth),args=way_points_smooth)["x"]
...
def target_speed_prediction(waypoints, num_waypoints_used=5,
    ...
    nn=abs(num_waypoints_used - 2 - curv_center)
    k=np.exp((-1)*exp_constant * nn)
    target_speed = 30 * k + 30
