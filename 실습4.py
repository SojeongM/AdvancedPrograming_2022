...
class LaneDetection:
    ...
    def cut_gray(self, state_image_full):
        ...
        '''
        gray_state_image= state_image_full[:65, :]
        gray_state_image = np.dot(gray_state_image[...,:3], [0.2989, 0.5870, 0.1140])
    ...
    def edge_detection(self, gray_image):
        ...
        grad=np.gradient(gray_image, axis=(0,1))
        aa=abs(grad[0])
        bb=abs(grad[1])
        gradient_sum=aa+bb
       ## thr_ind=
    ...
    def find_maxima_gradient_rowwise(self, gradient_sum):
        ...
        '''
        argmaxima=[]
        rag=gradient_sum.shape[0]
        for m in range(rag):
            p, _=find_peaks(gradient_sum[m], 3)
            argmaxima.append(p)
        return argmaxima
    ...
    def lane_detection(self, state_image_full):
        ...
        lane_1 = lane_boundary1_points
        lane_2 = lane_boundary2_points
        ...
        if lane_found:
            ...
            while r < 65:
                ...
                aaa=np.array((max_r-lane_1[0][0])*(max_r-lane_1[0][0]))
                bbb=np.array((max_r-lane_2[0][0])*(max_r-lane_2[0][0]))
                ...
                for i in range(0,n):
                   for j in range(0, n - i - 1):
                      if aaa[j] > aaa[j + 1]: 
                             s1_array[j], s1_array[j + 1] = s1_array[j + 1], s1_array[j]
                n=len(bbb)
                for i in range(0,n):
                   for j in range(0, n - i - 1):
                      if bbb[j] > bbb[j + 1]: 
                             s2_array[j], s2_array[j + 1] = s2_array[j + 1], s2_array[j]
                   
                
                aaa=np.argsort(aaa)
                bbb=np.argsort(bbb)
                print(bbb[0], s2_array[0])
                s1_arry=aaa[0]
                s2_arry=bbb[0]    
                p1=np.array([[max_r[s1_arry], r]])
                p2=np.array([[max_r[s2_arry], r]])
                r=r+1
                print(r)
                lane_boundary1_points=np.append(lane_boundary1_points, p1, 0)
                lane_boundary2_points=np.append(lane_boundary2_points, p2, 0)
                lane_1=p1
                lane_2=p2
