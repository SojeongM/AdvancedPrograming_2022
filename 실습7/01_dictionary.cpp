...
void makeGraph(const vector<string>& words) {
    ...
    for (int j = 1; j < words.size(); ++j) {
        ...
        for (int k = 0; k < len; ++k)
            if (words[j][k]!=words[i][k]) {    // 두 단어 words[i]와 words[j]의 k번째 알파벳이 서로 다를 때
...
void dfs(int here) {
    ...
    for (int there = 0; there < adj.size(); there++)
        if (seen[there]==0 && adj[here][there]==1)   // if (현재 정점에 대한 인접 정점이 있고, 해당 인접 정점을 방문한 적이 없으면)
...
vector<int> topologicalSort() {
    ...
    vector<int>::iterator iter_b = order.begin();
    vector<int>::iterator iter_e = order.end();
    
    reverse(order.begin(), order.end());
       // vector형 변수 order를 반전(reverse)시킴
