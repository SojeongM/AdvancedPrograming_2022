...
vector<int> getEulerTrailOrCircuit() {
    ...
    for (int i = 0; i < 26; i++)
        if (indegree[i]+1==outdegree[i]) {  // 현재 정점이 시작점일 조건
...
bool checkEuler() {
    ...
    else if (plus1 == 0 && minus1 == 0) {
        r = true;
    }
    else {
        r = false;
    }
    return r;  // 시작점과 끝점이 각각 하나씩 있거나 하나도 없으면 True 아니면 False
}
...
void makegraph(const vector<string>& words) {
    ...
    for (int i = 0; i < words.size(); i++) {    // 입력된 모든 단어에 대해 반복
        ...
        int b = words[i][size-1] - 'a';    // (i번째로 입력된 단어의 마지막 알파벳) - 'a'
